...
const OhMyQwencoderPlugin = async ({ client, directory, $ }) => {
    ...
    return {
        ...
        tool: allTools,
        "chat.message": pluginConfig.disabled_hooks.includes("chat.message") ? undefined : chatMessageHook,
        "tool.execute.after": pluginConfig.disabled_hooks.includes("tool.execute.after") ? undefined : toolExecuteAfterHook,
        stop: pluginConfig.disabled_hooks.includes("stop") ? undefined : stopHook,
        event: pluginConfig.disabled_hooks.includes("event") ? undefined : eventHook,
        "experimental.session.compacting": pluginConfig.disabled_hooks.includes("session.compacting") ? undefined : sessionCompactingHook,
    };
};
