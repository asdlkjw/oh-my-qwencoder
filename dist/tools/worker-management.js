...
import { getSession, genWorkerId, workerStatusTable, } from "./session.js";
export function createWorkerTools(client) {
    async function launchWorker(sid, workerId) {
...
dispatch_workers(workers: '[
  ...
  {"name":"게시판","feature":"CRUD API + list/detail pages","fileScope":["src/board/**","src/api/posts/**"],"sharedReadOnly":["src/lib/**"],"spec":"Build board with...","qaRequirements":"npm test -- --grep board"}
            ...
            async execute({ workers: workersJson }, ctx) {
                ...
                if (specs.length === 0)
                    return "❌ No workers specified.";
                if (specs.length > 8)
                    return "❌ Maximum 8 workers (vLLM concurrency limit).";
                const allScopes = [];
        ...
        worker_output: tool({
            ...
            async execute({ worker_id, wait }, ctx) {
                ...
                if (!worker)
                    return `❌ Worker ${worker_id} not found. Available: ${[...session.workers.keys()].join(", ")}`;
        ...
        worker_retry: tool({
            ...
            async execute({ worker_id, instructions }, ctx) {
                ...
                const worker = session.workers.get(worker_id);
                if (!worker)
                    return `❌ Worker ${worker_id} not found.`;
                worker.status = "retry";
                worker.retryCount++;
                worker.result = "";
...
//# sourceMappingURL=worker-management.js.map