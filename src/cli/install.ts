...
async function testConnection(baseURL: string, apiKey: string): Promise<{ ok: boolean; model?: string; error?: string }> {
  try {
...
function writeGlobalConfig(answers: VllmAnswers): string {
...
function registerPlugin(): string {
  const configDir = getGlobalConfigDir();
  ensureDir(configDir);
...
export async function runInstall(_args: string[]): Promise<void> {
  console.log(BANNER);
  ...
  try {
    ...
    if (existsSync(projectOpencode)) {
      try {
        ...
        if (!projConfig.plugin) projConfig.plugin = [];
